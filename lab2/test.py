...
def test(fun, x, y, eq=lambda x, y: x == y):
    ...
    if eq(z, y):
        ...
        if isinstance(y, (tuple, list, zip)): # membership test, not equality
            eq_repr = "\nfound among:\n   "
        else:
            eq_repr = "=="
...
def _run_tests(model, view=None):
    ...
    if fun := getattr(view, fname, None):
        ...
        # accepting both answers: keep as is, or subtract 6
        expected = [Vec(400,300), Vec(403,303)]
